...
namespace RazorPagesApp.Controllers
{
    ...
    public class HallApiController : ControllerBase
    {
        ...
        public HallApiController(ApplicationContext db)
        {
            context = db;
        }
        ...
        // Полный путь будет базовый путь ("api/Hall") + путь метода ("GetAll") = /api/Hall/GetAll
        [HttpGet("GetAll")]
        public async Task<IActionResult> GetAll()
        {
            var data = await context.SensorData_03
                .AsNoTracking()
        ...
        // Полный путь будет: /api/Hall/GetLast
        [HttpGet("GetLast")]
        public async Task<IActionResult> GetLast()
        {
            var lastData = await context.SensorData_03
                .AsNoTracking()
                .OrderByDescending(s => s.date)
                /*.Select(s => new
                {
