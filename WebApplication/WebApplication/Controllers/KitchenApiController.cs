...
namespace RazorPagesApp.Controllers
{
    ...
    public class KitchenApiController : ControllerBase
    {
        ...
        public KitchenApiController(ApplicationContext db)
        {
            context = db;
        }
        ...
        // Полный путь будет базовый путь ("api/Kitchen") + путь метода ("GetAll") = /api/Kitchen/GetAll
        [HttpGet("GetAll")]
        public async Task<IActionResult> GetAll()
        {
            var data = await context.SensorData_05
                .AsNoTracking()
        ...
        // Полный путь будет: /api/Kitchen/GetLast
        [HttpGet("GetLast")]
        public async Task<IActionResult> GetLast()
        {
            var lastData = await context.SensorData_05
                .AsNoTracking()
                .OrderByDescending(s => s.date)
                /*.Select(s => new
                {
