...
namespace RazorPagesApp.Controllers
{
    ...
    [Route("api/Hallway")] 
    public class HallwayApiController : ControllerBase
    {
        private readonly ApplicationContext context;

        // Внедрение зависимостей (Dependency Injection).
        // ASP.NET сам передаст сюда подключение к базе данных (ApplicationContext)
        public HallwayApiController(ApplicationContext db)
        {
            context = db;
        }
        ...
        // Полный путь будет базовый путь ("api/Hallway") + путь метода ("GetAll") = /api/Hallway/GetAll
        [HttpGet("GetAll")]
        public async Task<IActionResult> GetAll()
        {
            var data = await context.SensorData_04
                .AsNoTracking()
        ...
        // Полный путь будет: /api/Hallway/GetLast
        [HttpGet("GetLast")]
        public async Task<IActionResult> GetLast()
        {
            var lastData = await context.SensorData_04
                .AsNoTracking()
                .OrderByDescending(s => s.date)
                /*.Select(s => new
                {
