...
namespace RazorPagesApp.Controllers
{
    ...
    [Route("api/Bedroom")]
    public class BedroomApiController : ControllerBase
    {
        private readonly ApplicationContext context;

        // Внедрение зависимостей (Dependency Injection).
        // ASP.NET сам передаст сюда подключение к базе данных (ApplicationContext)
        public BedroomApiController(ApplicationContext db)
        {
            context = db;
        }
        ...
        // Полный путь будет базовый путь ("api/Bedroom") + путь метода ("GetAll") = /api/Bedroom/GetAll
        [HttpGet("GetAll")]
        public async Task<IActionResult> GetAll()
        {
            var data = await context.SensorData_01
                .AsNoTracking()
        ...
        // Полный путь будет: /api/Bedroom/GetLast
        [HttpGet("GetLast")]
        public async Task<IActionResult> GetLast()
        {
            var lastData = await context.SensorData_01
                .AsNoTracking()
                .OrderByDescending(s => s.date)
                /*.Select(s => new
                {
