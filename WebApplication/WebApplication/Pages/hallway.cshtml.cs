...
namespace RazorPagesApp.Pages
{
    public class hallwayModel : PageModel
    {
        ...
        public async Task OnGet() // тестить на IIS: public void OnGet()
        {
            ...
            ChartData.Columns.Add("Temperature", typeof(System.Single));
            // Add rows to data table
            for (int i = 0; i < SensorData_04.Count; i++)
            {
                ChartData.Rows.Add($"{SensorData_04[i].date.TimeOfDay.ToString().Substring(0, 5)}, {SensorData_04[i].date.Date.ToString().Substring(0, 5)}", (SensorData_04[i].temp));
            }
