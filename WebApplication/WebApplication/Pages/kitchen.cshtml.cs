...
namespace RazorPagesApp.Pages
{
    public class kitchenModel : PageModel
    {
        ...
        public async Task OnGet() // тестить на IIS: public void OnGet()
        {
            ...
            SensorData_05 = context.SensorData_05.AsNoTracking().OrderBy(p => p.date).ToList();
            ...
            {
                ChartData.Rows.Add($"{SensorData_05[i].date.TimeOfDay.ToString().Substring(0, 5)}, {SensorData_05[i].date.Date.ToString().Substring(0, 5)}", (SensorData_05[i].temp));
            }
